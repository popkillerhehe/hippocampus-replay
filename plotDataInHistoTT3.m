function []=plotDataInHistoTT3(rw_data_bin, vr_data_bin, bin_size, data_str)
%% Loading
...
vr_firing_rate = mean(vr_data_bin.spike, 2);
vr_immobile_normalized_firing_rate = mean(vr_data_bin.spike(:,vr_immobile_idx) ./ repmat(vr_firing_rate, 1, sum(vr_immobile_idx)));
...
rw_firing_rate = mean(rw_data_bin.spike, 2);
rw_immobile_normalized_firing_rate = mean(rw_data_bin.spike(:,rw_immobile_idx) ./ repmat(rw_firing_rate, 1, sum(rw_immobile_idx)));
...
filename = sprintf('histo3_%s_stationary.pdf', data_str);
...
vr_mobile_normalized_firing_rate = mean(vr_data_bin.spike(:,not(vr_immobile_idx)) ./ repmat(vr_firing_rate, 1, sum(not(vr_immobile_idx))));
...
rw_mobile_normalized_firing_rate = mean(rw_data_bin.spike(:,not(rw_immobile_idx)) ./ repmat(rw_firing_rate, 1, sum(not(rw_immobile_idx))));
...
filename = sprintf('histo3_%s_mobile.pdf', data_str);
